...
## 캐릭터 스프라이트 해상도 (미정 — 참고 자료만 기록)

2026-08-10 웹 조사 기준. 공식 스펙이 공개된 사례는 거의 없어 대부분 커뮤니티 추정치/실측 확인 혼재. **그리드 칸 크기([[03_combat_system]], 아직 미정)와 묶어서 결정해야 함** — 캐릭터가 칸에서 차지하는 비율이 실제 체감 크기를 좌우.

| 스타일 | 캐릭터 세로 높이(추정) | 특징 |
|---|---|---|
| 정통 레트로 (Chained Echoes류, 실측 확인: 64x64 셀 안에 약 12x21px) | 20~32px | SNES 감성, 제작 부담 적음 |
| 중간 (Sea of Stars류로 추정, 미확인) | 40~56px | 디테일 있으면서도 관리 가능 |
| 풀 HD-2D (옥토패스 트래블러류로 추정, 미확인) | 64px 이상 | 표정/의상 디테일 풍부, 제작 부담 큼 |

**추천 시작점: 48px 안팎.** 1인 개발 + 콘텐츠 부담을 계속 고려해온 프로젝트 방향(유파 6~9개 제한 등)에 맞춰, 옥토패스급 풀 디테일보다는 적당히 디테일을 살리면서 제작 부담은 낮춘 중간 지점을 추천. 2026-08-10 테스트로 그린 32x32(`assets/sprites/character_test.png`)는 이 조사 이전의 임의 테스트값이며 확정 크기 아님.
